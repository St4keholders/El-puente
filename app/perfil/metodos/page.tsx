...
export default function MetodosPagoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (provider.trim().length < 2) {
      setErrorMsg("Ingresa el banco o plataforma (ej. Bancolombia, Nequi, etc.).");
      return;
    }
    if (accountHolder.trim().length < 2) {
      setErrorMsg("Ingresa el nombre de la persona titular de la cuenta.");
      return;
    }
    if (accountValue.trim().length < 3) {
      setErrorMsg("Ingresa el número de cuenta, teléfono o enlace de pago.");
      return;
    }
    ...
    try {
      if (editingMethod) {
        ...
        // Si la casilla está marcada, sincronizar en causas activas y borradores
        if (syncActiveCauses) {
        ...
        // Crear nuevo método (posición al final)
        ...
        if (insertError) {
          if (insertError.message.includes("LIMITE_METODOS")) {
            setErrorMsg("Llegaste al máximo de 10 métodos permitidos.");
            setSaving(false);
            return;
          }
          throw insertError;
      ...
      setErrorMsg(err.message || "Error al guardar el método.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ==========================================================
          MODAL DE AGREGAR / EDITAR MÉTODO
          ========================================================== */}
      {isModalOpen && (
        <div
          role="dialog"
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-left">
              ...
              {/* Casilla de sincronización al editar */}
              {editingMethod && (
                <div className="pt-2">
